...
@dataclass
class DeformableMirror3D(OpticalElement3D):
    """
    Geometric deformable mirror / adaptive optics corrector.

    The DM is treated as an OPD-producing optical element on a plane. It does
    not diffract or physically reshape the beam; it adds an optical path
    difference sampled from an actuator influence-function model.

    Important convention
    --------------------
    ``opd_per_command_m`` is the *optical path difference* response per unit
    command at the peak of one actuator influence function.

    For a reflective DM, if you instead know surface displacement per command
    ``h_m_per_cmd``, then approximately

        opd_per_command_m = 2 * h_m_per_cmd * cos(incidence_angle)

    should be passed in.

    Coordinates
    -----------
    Actuator centres are defined in the local coordinates of the optical
    element plane. The local axes are inherited from ``OpticalElement3D`` via
    ``plane_basis()``.

    Parameters
    ----------
    point, normal:
        Plane location and normal, inherited from ``OpticalElement3D``.
    actuator_shape:
        Number of actuators as ``(ny, nx)``.
    actuator_pitch_m:
        Actuator pitch in metres.
    opd_per_command_m:
        Peak OPD response per actuator command unit.
    commands:
        Actuator command array with shape ``actuator_shape``. If None, zeros
        are used.
    valid_actuator_mask:
        Boolean mask of active/valid actuators. Useful for missing-corner DMs.
    influence:
        Influence-function model. Currently supports ``"gaussian"`` and
        ``"nearest"``. A custom callable can be supplied via
        ``influence_function``.
    coupling:
        Nearest-neighbour coupling for the Gaussian influence model. If
        ``coupling=0.3``, then a single actuator influence is 0.3 at one pitch.
    influence_sigma_m:
        Explicit Gaussian sigma. If supplied, this overrides ``coupling``.
    influence_function:
        Optional callable ``f(r_m) -> response`` for custom influence functions.
    normalize_influence:
        If True, normalise by the sum of influence functions at each point.
        Usually False for physical actuator superposition.
    command_limits:
        Optional ``(min_cmd, max_cmd)`` clipping limits.
    clear_radius:
        Optional circular clear aperture radius in local coordinates.
    aperture_stop:
        If True, rays outside ``clear_radius`` are marked dead.
    """

    actuator_shape: Tuple[int, int] = (12, 12)
    actuator_pitch_m: float = 1.0e-3
    opd_per_command_m: float = 1.0e-6
    ...
    def __post_init__(self):
        ...
        if self.clear_radius is None:
            # Covers the full actuator grid, plus half a pitch.
            self.clear_radius = 0.5 * max(nx, ny) * self.actuator_pitch_m
    ...
    def actuator_centres(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Return actuator centre coordinates ``(u, v)`` in the DM local plane.

        Returns
        -------
        uu, vv:
            2D arrays with shape ``actuator_shape``.
        """
        ny, nx = self.actuator_shape

        u = (np.arange(nx) - 0.5 * (nx - 1)) * self.actuator_pitch_m
        v = (np.arange(ny) - 0.5 * (ny - 1)) * self.actuator_pitch_m
    ...
    def set_commands(self, commands: np.ndarray):
        ...
        Commands are clipped to ``command_limits`` if limits are supplied.
        """
    ...
    def _gaussian_sigma(self) -> float:
        """
        Return Gaussian sigma for the influence function.

        For influence I(r) = exp[-r^2/(2 sigma^2)], nearest-neighbour coupling
        at r = pitch is

            coupling = exp[-pitch^2/(2 sigma^2)].
        """
        if self.influence_sigma_m is not None:
            return float(self.influence_sigma_m)

        if self.coupling is None:
            return 0.4 * self.actuator_pitch_m
        ...
        return self.actuator_pitch_m / np.sqrt(-2.0 * np.log(coupling))

    def sample_uv(self, uv: np.ndarray, t: float = 0.0) -> Tuple[np.ndarray, np.ndarray]:
        """
        Sample the DM OPD at local plane coordinates.

        Parameters
        ----------
        uv:
            Array with final dimension 2, containing local coordinates in metres.
        t:
            Included for API compatibility with rotating phase screens.
        ...
        opd:
            Sampled OPD [m].
        valid:
            Boolean validity mask. If ``clear_radius`` is set, valid means
            inside the circular DM clear aperture.
        """
        uv = np.asarray(uv, dtype=float)
        original_shape = uv.shape[:-1]
        ...
        r_ap = np.sqrt(u**2 + v**2)
        valid = r_ap <= float(self.clear_radius)

        opd = np.zeros(uv_flat.shape[0], dtype=float)
        ...
        if self.influence == "nearest" and self.influence_function is None:
            ...
            if centres.size == 0:
                return opd.reshape(original_shape), valid.reshape(original_shape)

            for i, (ui, vi) in enumerate(zip(u, v)):
                d2 = (centres[:, 0] - ui) ** 2 + (centres[:, 1] - vi) ** 2
                j = int(np.argmin(d2))
                opd[i] = self.opd_per_command_m * cmds[j]

        else:
            uu_act, vv_act = self.actuator_centres()
            cmds = self.commands
            active = self.valid_actuator_mask

            sigma = self._gaussian_sigma()

            weight_sum = np.zeros_like(opd)
            ...
            for iy in range(self.actuator_shape[0]):
                for ix in range(self.actuator_shape[1]):
                    ...
                    if self.influence_function is not None:
                        ...
                        raise ValueError(
                            "Unsupported influence model. Use 'gaussian', "
                            "'nearest', or provide influence_function."
                        )

                    opd += self.opd_per_command_m * cmds[iy, ix] * infl
                    weight_sum += infl

            if self.normalize_influence:
                good = weight_sum > 1e-30
                opd[good] /= weight_sum[good]
    ...
    def opd_map_on_grid(
        ...
        """
        Sample the DM OPD on a local-coordinate grid.
        """
    ...
    def apply(self, ray: Ray3D, t: float = 0.0):
        """
        Apply DM OPD to a ray.

        This updates ``ray.opd`` and moves the ray to the DM plane. It does not
        change ray direction, because this geometric model treats the DM as a
        phase/OPD element rather than a slope-reflecting surface.
        """
        p, _ = self.intersect_point(ray)
        ...
        if not bool(valid[0]) and self.aperture_stop:
            ray.alive = False
            return ray
...
class ShackHartmannWFS3D(OpticalElement3D):
    """
    Geometric Shack-Hartmann WFS handoff plane.

    This class stores the geometry of a Shack-Hartmann lenslet array as an
    ``OpticalElement3D`` so it can be placed in an ``OpticalBench3D``. It is
    intentionally mostly geometric: it does not itself perform Fresnel
    propagation or generate spots.

    The intended workflow is:

        OpticalBench3D / beam_trace
            -> sample beam as Wavefront2D at this SH-WFS plane
            -> apply DM / Gaussian illumination if needed
            -> use shwfs.py or wave_optics.py to propagate each lenslet
               subaperture to its focal spot

    Parameters
    ----------
    point, normal:
        Physical plane of the lenslet array.
    n_lenslets:
        Number of lenslets ``(ny, nx)``.
    lenslet_pitch_m:
        Lenslet pitch in metres.
    lenslet_diameter_m:
        Clear aperture diameter of each lenslet. If None, uses
        ``lenslet_fill_factor * lenslet_pitch_m``.
    lenslet_fill_factor:
        Used only if ``lenslet_diameter_m`` is None.
    lenslet_shape:
        ``"square"`` or ``"circular"`` lenslet aperture.
    focal_length_m:
        Lenslet focal length.
    wavelength_m:
        Nominal WFS wavelength.
    detector_pixel_pitch_m:
        Detector pixel pitch in metres.
    pixels_per_lenslet:
        Detector sampling allocated to each lenslet spot.
    valid_lenslet_mask:
        Optional boolean mask of usable lenslets.
    read_noise_e, background_e, quantum_efficiency, saturation_e:
        Detector/noise metadata for later SH-WFS simulation.
    terminate_rays:
        If True, rays are marked dead after reaching this sensor plane.
    """
    ...
    focal_length_m: float = 20.0e-3
    wavelength_m: float = 633.0e-9

    detector_pixel_pitch_m: float = 5.0e-6
    pixels_per_lenslet: int = 32

    valid_lenslet_mask: Optional[np.ndarray] = None
    reference_centroids_pix: Optional[np.ndarray] = None
    ...
    def __post_init__(self):
        ...
        if self.reference_centroids_pix is None:
            self.reference_centroids_pix = self.default_reference_centroids_pix()

    @property
    def detector_shape(self) -> Tuple[int, int]:
        """
        Detector shape ``(ny_pix, nx_pix)`` needed to hold all lenslet cells.
        """
        ny, nx = self.n_lenslets
        return (
            ny * self.pixels_per_lenslet,
            nx * self.pixels_per_lenslet,
        )

    @property
    def clear_radius(self) -> float:
        """
        Approximate radius enclosing the full lenslet grid.
        """
        ny, nx = self.n_lenslets
        half_x = 0.5 * nx * self.lenslet_pitch_m
        half_y = 0.5 * ny * self.lenslet_pitch_m
    ...
    def lenslet_centres(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Return lenslet centre coordinates in local plane coordinates.

        Returns
        -------
        uu, vv:
            2D arrays with shape ``n_lenslets``.
        """
        ny, nx = self.n_lenslets
    ...
    def default_reference_centroids_pix(self) -> np.ndarray:
        """
        Reference detector centroids at the centre of each lenslet cell.

        Returns
        -------
        ref:
            Array with shape ``(ny_lenslets, nx_lenslets, 2)`` containing
            ``(x_pix, y_pix)``.
        """
        ny, nx = self.n_lenslets
        ref = np.zeros((ny, nx, 2), dtype=float)

        for iy in range(ny):
            for ix in range(nx):
                ref[iy, ix, 0] = (ix + 0.5) * self.pixels_per_lenslet
                ref[iy, ix, 1] = (iy + 0.5) * self.pixels_per_lenslet

        return ref

    def lenslet_index_for_uv(
        self,
        uv: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Map local coordinates to nearest lenslet indices.

        Parameters
        ----------
        uv:
            Array with final dimension 2, containing local coordinates [m].
        ...
        if self.lenslet_shape == "square":
            in_ap = (np.abs(du) <= half) & (np.abs(dv) <= half)
        else:
            in_ap = (du**2 + dv**2) <= half**2
    ...
    def lenslet_mask_for_grid(
        ...
        """
        Return a boolean aperture mask for one lenslet on a coordinate grid.

        Parameters
        ----------
        xx, yy:
            Local coordinate grids [m].
        lenslet_index:
            ``(iy, ix)`` lenslet index.
        """
        iy, ix = lenslet_index
    ...
    def detector_cell_bounds(self, lenslet_index: Tuple[int, int]) -> Tuple[int, int, int, int]:
        """
        Return detector cell bounds for one lenslet.
    ...
    def apply(self, ray: Ray3D, t: float = 0.0):
        """
        Move a ray to the SH-WFS plane.

        If ``terminate_rays=True``, the ray is marked dead after reaching the
        sensor. This is useful for visualisation and geometric tracing.
        """
        p, _ = self.intersect_point(ray)
